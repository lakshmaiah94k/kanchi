...
def student_registration(request):
    if request.method == "POST":
        ...
        return HttpResponseRedirect(reverse('college:index'))
    return render(request, 'college/student_registration.html')
...
def staff_registration(request):
    if request.method == 'POST':
        ...
        return HttpResponseRedirect(reverse('college:index'))
    return render(request, 'college/staff_registration.html')
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if hasattr(user, 'studentreg'):
                return HttpResponseRedirect(reverse('college:details'))
            elif hasattr(user, 'staff'):
                return HttpResponseRedirect(reverse('college:details'))
        else:
            return render(request, 'college/login.html', {'error_message': "Invalid Credentials given"})
...
def details(request):
    user = request.user
    if hasattr(user, 'studentreg'):
        return render(request, 'college/student_details.html', {'stu_det': request.user.studentreg})
    else:
        return render(request, 'college/staff_details.html', {'sta_det': request.user.staff})


def student_staff_list(request):
    dep = request.user.studentreg.department
    st_list = Staff.objects.filter(department=dep)
    return render(request, 'college/student_staff_list.html', {'st_list': st_list})
